...
| total_chunks | int | 195, 221, ... | Tổng số chunk của văn bản gốc; dùng để tính vị trí tương đối của chunk trong tài liệu |
...
Kết quả chạy `ChunkingStrategyComparator().compare()` trên 2-3 tài liệu:

| Tài liệu | Strategy | Chunk Count | Avg Length | Preserves Context? |
|-----------|----------|-------------|------------|-------------------|
| 001_luat_52656.md | fixed_size | 133 | 199.44 | Trung bình, chunk đều nhưng có thể cắt ngang điều/khoản |
| 001_luat_52656.md | by_sentences | 73 | 361.00 | Khá tốt, giữ nguyên câu nhưng chunk dài hơn fixed-size |
| 001_luat_52656.md | recursive | 221 | 118.23 | Tốt, ưu tiên đoạn/dòng/câu nên giữ cấu trúc tự nhiên hơn |
| 002_nghi_dinh_8908.md | fixed_size | 119 | 198.58 | Trung bình, kích thước ổn định nhưng dễ cắt ngang ngữ cảnh pháp lý |
| 002_nghi_dinh_8908.md | by_sentences | 50 | 470.40 | Khá tốt, giữ nguyên câu nhưng chunk khá dài |
| 002_nghi_dinh_8908.md | recursive | 192 | 121.35 | Tốt, tận dụng xuống dòng và cấu trúc đoạn của văn bản |
| 003_nghi_dinh_14849.md | fixed_size | 99 | 199.27 | Trung bình, dễ kiểm soát kích thước nhưng chưa bám sát cấu trúc văn bản |
| 003_nghi_dinh_14849.md | by_sentences | 42 | 467.00 | Khá tốt, giữ nguyên câu nhưng chunk khá dài |
| 003_nghi_dinh_14849.md | recursive | 162 | 120.06 | Tốt hơn cho văn bản pháp luật vì ưu tiên chia theo cấu trúc tự nhiên trước |

**Nhận xét:** Sau khi thêm bước tiền xử lý (`preprocess_legal_markdown`) để loại bỏ artifact Markdown (dòng phân cách, ký hiệu rỗng), `RecursiveChunker` cho kết quả ổn định hơn rất nhiều (221 chunk, avg 118 chars thay vì hàng nghìn chunk rác trước đây). `SentenceChunker` tạo chunk ít nhất nhưng dài nhất (avg 361–470 chars); `FixedSizeChunker` tạo chunk đều đặn nhất (~199 chars); `RecursiveChunker` bám sát cấu trúc đoạn/dòng của văn bản tốt hơn hai strategy còn lại.
...
```python
from src.chunking import RecursiveChunker, preprocess_legal_markdown
...
Chạy trên 3 tài liệu pháp luật với chunk_size=200 (sau tiền xử lý):

| Tài liệu | Strategy | Chunk Count | Avg Length | Retrieval Quality? |
|-----------|----------|-------------|------------|--------------------|
| 001_luat_52656.md | FixedSizeChunker | 133 | 199.44 | Trung bình, chunk đều nhưng có thể cắt ngang điều/khoản |
| 001_luat_52656.md | **RecursiveChunker của tôi** | **221** | **118.23** | Tốt hơn về bảo toàn cấu trúc đoạn/dòng/câu, nhưng số chunk nhiều hơn |
| 002_nghi_dinh_8908.md | FixedSizeChunker | 119 | 198.58 | Trung bình, kích thước ổn định nhưng dễ cắt ngang ngữ cảnh pháp lý |
| 002_nghi_dinh_8908.md | **RecursiveChunker của tôi** | **192** | **121.35** | Tốt hơn nhờ tận dụng xuống dòng và cấu trúc đoạn của văn bản |
| 003_nghi_dinh_14849.md | FixedSizeChunker | 99 | 199.27 | Trung bình, dễ kiểm soát kích thước nhưng chưa bám sát cấu trúc văn bản |
| 003_nghi_dinh_14849.md | **RecursiveChunker của tôi** | **162** | **120.06** | Tốt hơn cho văn bản pháp luật vì ưu tiên chia theo cấu trúc tự nhiên trước |

RecursiveChunker tạo nhiều chunk hơn FixedSizeChunker (avg ~120 chars vs avg ~199 chars) vì tách theo ranh giới đoạn và dòng thay vì cắt cứng theo kích thước. Điều này giúp bảo toàn cấu trúc tự nhiên của văn bản pháp luật — mỗi chunk thường tương ứng một Điều hoặc Khoản — nhưng số chunk nhiều hơn đáng kể.

### So Sánh Với Thành Viên Khác

| Thành viên | Strategy | Retrieval Score (/10) | Điểm mạnh | Điểm yếu |
|-----------|----------|----------------------|-----------|----------|
| Tôi | RecursiveChunker | 9/10 | Giữ cấu trúc đoạn, dòng, câu; phù hợp văn bản pháp luật dài | Có thể tạo chunk dài, cần chọn chunk_size hợp lý |
| Do Thi Thanh Binh | FixedSizeChunker | 7/10 | Dễ implement, chunk đều, dễ kiểm soát kích thước | Dễ cắt ngang điều khoản hoặc câu quan trọng |
| Le Trung Kien | SentenceChunker | 8/10 | Giữ nguyên câu, dễ đọc hơn fixed-size | Không tận dụng tốt cấu trúc chương/mục/điều và xuống dòng |

**Strategy nào tốt nhất cho domain này? Tại sao?**

RecursiveChunker (9/10) phù hợp nhất cho văn bản pháp luật Việt Nam vì ưu tiên giữ nguyên cấu trúc đoạn → dòng → câu trước khi cắt theo từ. Điều này đặc biệt có giá trị với tài liệu pháp luật có cấu trúc Chương/Điều/Khoản rõ ràng — mỗi chunk thường tương ứng một đơn vị ngữ nghĩa độc lập. SentenceChunker (8/10) cũng tốt và ổn định hơn với dữ liệu có artifact, nhưng không tận dụng được cấu trúc Markdown phân cấp của tài liệu.
...
Dùng `re.split()` với **lookbehind** (`(?<=\.)`, `(?<=!)`, `(?<=\?)`) để tách ngay sau các dấu kết thúc câu, giữ dấu câu cùng với câu đó. Sau khi tách, gom nhóm `max_sentences_per_chunk` câu liên tiếp thành một chunk và join bằng dấu cách. Edge case xử lý: câu rỗng sau strip được bỏ qua, max_sentences tối thiểu là 1.
...
**Nhận xét:** Retrieval hoàn toàn thất bại vì hai lý do cộng hưởng: (1) MockEmbedder tạo random vectors không phản ánh ngữ nghĩa tiếng Việt; (2) RecursiveChunker (chunk_size=500) vẫn tạo nhiều chunk cực ngắn từ artifact PDF — riêng `001_luat_52656.md` có 33 chunk < 50 ký tự (từ lẻ như `"cùng"`, `"hồi"`); với random scoring, chunk rác ngẫu nhiên được score cao nhất. Filter theo `document_type=luat` hoạt động đúng (chỉ trả về chunks từ 001_luat_52656.md), nhưng chưa cải thiện được relevance. *(Benchmark này chạy trước khi thêm `preprocess_legal_markdown`.)*
...
Thành viên dùng `SentenceChunker` chỉ ra rằng văn bản pháp luật Việt Nam thường viết mỗi khoản là một câu dài — SentenceChunker tách đúng theo ranh giới này và tạo ra chunk ổn định hơn (73 chunk, avg 361 chars) so với RecursiveChunker không có tiền xử lý (195 chunk, avg 134 chars, còn 33 chunk < 50 chars). Điều này dẫn tôi đến quyết định implement `preprocess_legal_markdown` để loại bỏ artifact trước khi chunking — sau tiền xử lý, RecursiveChunker cho 221 chunk ổn định hơn (avg 118 chars) và giữ cấu trúc phân cấp pháp luật tốt hơn SentenceChunker.
...
Ba thay đổi chính: (1) **Tiền xử lý dữ liệu** — ✅ đã implement `preprocess_legal_markdown` (loại bỏ dòng phân cách, ký hiệu rỗng), giảm chunk rác đáng kể; bước tiếp theo là filter chunk < 50 ký tự sau chunking; (2) **Dùng Vietnamese embedder** — MockEmbedder hoàn toàn vô dụng với tiếng Việt, cần multilingual sentence-transformers hoặc embedder tiếng Việt chuyên biệt để benchmark có ý nghĩa; (3) **Thêm metadata `article_id`** để truy xuất từng Điều cụ thể khi người dùng hỏi theo số Điều — đây là use case rất phổ biến với tài liệu pháp luật.

### Failure Case Analysis

**3 queries thất bại hoàn toàn (Q1, Q3, Q4):** Top-1 retrieved chunks đều là artifact Markdown — từ lẻ (`"cùng"`, `"hồi"`) hoặc dòng phân cách (`"--------"`), không chứa nội dung pháp lý nào. *(Benchmark chạy với chunk_size=500 trước khi thêm preprocessing.)*

**Nguyên nhân kép:**
- **Embedder:** MockEmbedder gán random score nên artifact ngắn ngẫu nhiên được score cao (0.39–0.47)
- **Chunker:** RecursiveChunker (chunk_size=500, không preprocessing) tạo 4,205 chunks tổng với 33 chunk < 50 chars chỉ riêng `001_luat_52656.md` — xác suất chunk rác xuất hiện ở top-1 rất cao do random scoring

**Cải thiện đã thực hiện và kế hoạch:**
1. ✅ Đã implement `preprocess_legal_markdown` — loại bỏ dòng phân cách và ký hiệu rỗng; RecursiveChunker giờ cho 221 chunk ổn định hơn ở chunk_size=200
2. Dùng Vietnamese sentence-transformers để có semantic similarity thực sự (cải thiện lớn nhất)
3. Thêm metadata `article_id` + `chapter_id` để hỗ trợ filter theo Điều/Chương
4. Filter chunk < 50 ký tự sau chunking để loại bỏ fragment còn sót lại
